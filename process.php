<?php
   ...
   try {
       ...
       $stmt = $pdo->prepare("INSERT INTO submissions (wallet, email, data, submission_type, created_at) VALUES (?, ?, ?, ?, NOW())");
       $stmt->execute([$wallet, $email, $data, $submission_type]);
   } catch (Exception $e) {
   ...
   try {
       $pdo->exec("DELETE FROM submissions WHERE created_at < NOW() - INTERVAL 30 DAY");
   } catch (Exception $e) {
